...
# don't know what the follwing line 56 was for nor why it was deactivated
...
if __name__ == '__main__':
    ...
    for iDs in range(len(datasets)):
        ...
        #        plt.imshow(cv2.cvtcolor(img, cv2.color_BGR2RGB))
            ...
            model = get_unet()
            # use_weights = True;
            model_checkpoint = ModelCheckpoint(data_path + '/RGB_NET128.hdf5', monitor = 'val_loss', save_best_only = True)

            print('-' * 30)
            print('Fitting model...')
            print('-' * 30)
            history = model.fit(imgs_train_sp_aug, masks_train_sp_aug, batch_size = 8, epochs = 250, verbose = 1, 
                                shuffle = True, validation_data = (imgs_val_sp_aug, masks_val_sp_aug),
                                callbacks = [model_checkpoint])
            '''
            history = model.fit(imgs_train_sp_aug, masks_train_sp_aug, batch_size = 4, epochs = 200, verbose = 1, shuffle = True,
                                validation_split = 0.05, callbacks = [model_checkpoint])
            '''
    ...
    #%%
        ...
        if test_only == True:
            model = get_unet()
            model.load_weights(data_path + '/RGB_NET128.hdf5')
        datasets_save = ['set1c']
        # load testing data
